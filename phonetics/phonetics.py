...
class Phonetics(commands.Cog):
    ...
    async def codeword(self, ctx, letter: str = None):
        ...
        try:
            ...
            await ctx.send(
                "Syntax error: letter must be in range (A-Z). No special characters or numbers allowed."
            )
    ...
    async def phonetic(self, ctx, letter=None):
        ...
        try:
            await ctx.send(
                letter + ": " + self.ph[letter][0] + " " + self.ph[letter][1]
            )
        except KeyError:
            await ctx.send(
                "Syntax error: letter must be in range (A-Z). No special characters or numbers allowed."
    ...
    async def phtrivia(self, ctx):
        ...
        delta = (
            '"Delta" is replaced by "Data", "Dixie", or "David" at airports that have a majority of '
